...
export const approveAccount: RequestHandler = async (req: IRequest, res, next) => {
  ...
  try {
    ...
    // Delete approval notifications for this user if approved
    if (status === "active") {
      await Notification.deleteMany({
        type: "approval",
        createdFor: { $in: [user._id] }
      });
    }
...
export const getDashboardStats: RequestHandler = async (req: IRequest, res, next) => {
  try {
    ...
    // Fetch all users for this school and populate role
    const users = await User.find({ school: adminSchool._id }).populate("role", "type");
    const classes = await Class.find({ school: adminSchool._id });
    // Optionally, fetch courses if you want to count them
    // const courses = await Course.find({ classid: { $in: classes.map(cls => cls._id) } });

    const totalTeachers = users.filter(u => u.role && (u.role as any).type === "teacher").length;
    const totalStudents = users.filter(u => u.role && (u.role as any).type === "student").length;
    const activeTeachers = users.filter(u => u.role && (u.role as any).type === "teacher" && u.status === "active").length;
    const activeStudents = users.filter(u => u.role && (u.role as any).type === "student" && u.status === "active").length;
    const inactiveTeachers = totalTeachers - activeTeachers;
    const inactiveStudents = totalStudents - activeStudents;
    const pendingApprovals = users.filter(u => u.status === "pending").length;
    const totalClasses = classes.length;
    // If you want to count courses:
    // const totalCourses = courses.length;
    // Or, if each class has a courses array:
    const totalCourses = classes.reduce((sum, cls) => sum + (Array.isArray((cls as any).courses) ? (cls as any).courses.length : 0), 0);
    const classesWithTeachers = classes.filter(cls => Array.isArray((cls as any).teachers) && (cls as any).teachers.length > 0).length;
...
// Add missing exports for usersList, coursesList, and classesList
export const usersList = (role: "student" | "teacher"): RequestHandler => {
  return async (req: IRequest, res, next) => {
    try {
      ...
      // Populate classname for students, just role for teachers
      let query = User.find({ school: school._id }).populate("role", "type");
      if (role === "student") {
        query = query.populate("classname", "name");
      }
...
export const classesList: RequestHandler = async (req: IRequest, res, next) => {
  ...
  try {
    ...
    if (!schoolId) throw new HttpError(400, "School not found");
    const classItems = await Class.find({ school: schoolId }).populate("courses");
    res.status(200).json({
